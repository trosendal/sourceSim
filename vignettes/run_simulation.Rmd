...
*Campylobacter jejuni*. In the package, there is are sample sequences
of the 7 housekeeping genes downloaded from pubMLST that are used for
7-allele MLST. First load those sequences to calculate the base
frequency of *C. jejuni*:
...
sequences only the base frequencies.
...
The `data.frame` represents the distribution of genotypes in each of
the three metapopulations in the last generation of the
simulation. Every row in the resulting `data.frame` is one unique
genotype that has been produced in the simulation, with unique
identifiers and the frequency in each metapopulation. A generated MLST
name is also given which does not correspond to the PubMLST
nomenclature but is just a convenient identifier. If you inspect the
species you will notice that there is no overlap between the 3
metapopulations. This is because shared membership of a genotype in
multiple metapopulations can only arise by random chance without
migration.
...
represents the migration from population i to population j.
...
the results in a phylogenetic graph:
